...
@mcp.tool()
async def get_inventory(item: str) -> dict:
    return {"item": item, "qty": 10}
...
#uv run mcp install inventory-mcp-server.py   
#uv run --with "mcp[cli]"" mcp install run  inventory-mcp-server.py  
# uv run --with mcp[cli] mcp run /.../inventory-mcp-server.py
